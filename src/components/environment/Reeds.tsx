...
/**
 * Reed blades ringing the pond, drawn in a single instanced mesh and bent by a
 * shared wind shader (each blade gets a random phase so the sway desyncs).
 */
export function Reeds() {
  ...
  const { geometry, matrices } = useMemo(() => {
    const g = new THREE.CylinderGeometry(0.0, 0.07, 1.4, 5, 1);
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      const r = WORLD.playableRadius - 2 + Math.random() * 4;
      dummy.position.set(Math.cos(angle) * r, 0, Math.sin(angle) * r);
      dummy.rotation.set(0, Math.random() * Math.PI * 2, 0);
      const hy = 0.8 + Math.random() * 1.1;
      const hx = 0.8 + Math.random() * 0.5;
